...
const NavbarUser= () => {
  ...
  return (
    <nav className='flex flex-col md:flex-row justify-between w-full shadow-lg'>
      <div className={`flex flex-row justify-between items-center px-5 py-3 ${isMenuOpen ? 'bg-transparent' : 'bg-white'} md:bg-white md:w-1/6 md:justify-center `}>
        ...
        <svg
            ...
            className="h-8 w-auto bg-transparant"
        >
            <path
      ...
      <div className={`nav-links flex flex-col md:flex-row justify-between md:items-center bg-white px-5 py-3 m-5 md:m-0 rounded-lg md:rounded-none ${isMenuOpen ? 'block' : 'hidden'} md:flex md:w-full`}>
        ...
        <li><a href="#" className={`text-black text-base font-medium ${activeMenuItem === 2 ? 'text-amber-400' : 'hover:text-amber-300'}`} onClick={() => handleMenuItemClick(2)}>Home</a></li>
          <li className='relative'>
            <button onClick={onToggleProfileDropdown} className={`text-black text-base font-medium ${activeMenuItem === 1 ? 'text-amber-400' : 'hover:text-amber-300'} relative`}>Produk</button>
      ...
      <div className={`nav-auth flex flex-col md:flex-row justify-between md:items-center bg-red-500 px-5 py-3 m-5 md:m-2 rounded-lg md:rounded-none ${isMenuOpen ? 'block' : 'hidden'} md:flex md:w-1/4`}>
        <ul className='flex flex-col md:flex-row gap-3 md:gap-5 md:items-center md:mx-auto'>
          <li><button href={route('login')} className={`bg-amber-400 px-3 py-2 rounded-lg text-white text-base font-medium ${activeMenuItem === 4 ? 'text-black' : 'hover:text-white'}`} onClick={() => handleMenuItemClick(4)}>Login</button></li>
          <li><a href={route('register')} className={`text-black text-base font-medium ${activeMenuItem === 5 ? 'text-amber-400' : 'hover:text-amber-300'}`} onClick={() => handleMenuItemClick(5)}>Register</a></li>
        </ul>
      </div>
    </nav>
  );
